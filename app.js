...
const gameStarts={};
const leaderboardApi='/.netlify/functions/leaderboard';
...
function setTheme(theme){const coastal=theme==='coastal';document.body.classList.toggle('coastal-theme',coastal);localStorage.setItem('babyTheme',coastal?'coastal':'classic');const button=$('#themeToggle');if(button){button.textContent=coastal?'Classic':'Coastal';button.setAttribute('aria-pressed',coastal)}}
setTheme(localStorage.getItem('babyTheme')==='coastal'?'coastal':'classic');
$('#themeToggle')?.addEventListener('click',()=>setTheme(document.body.classList.contains('coastal-theme')?'classic':'coastal'));
...
function showGameIntro(game){const info=gameInstructions[game],dialog=$('#modal');dialog.classList.add('game-intro');$('#modalContent').innerHTML=`<p class="eyebrow">How to play</p><h2>${info.title}</h2><p>${info.text}</p><p class="timer-note">Your timer starts when you press the button.</p><div class="intro-actions"><button class="start-game-button" id="startGameButton">Start game</button><button class="intro-back-button" id="introBackButton">Back to games</button></div>`;dialog.showModal();$('#startGameButton').onclick=()=>{gameStarts[game]=Date.now();updateTimers();dialog.classList.remove('game-intro');dialog.close()};$('#introBackButton').onclick=()=>{dialog.classList.remove('game-intro');dialog.close();showView('home')}}
...
$('#clearLeaderboard').onclick=async()=>{
  const password=isLocal?'reset':String(prompt('Enter the host password to clear all scores:')||'').trim().toLowerCase();
  if(!password)return;
  try{
